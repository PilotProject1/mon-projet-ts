 ...
 *
 * Les classes « at- » n'habillent rien : elles désignent ce que les
 * animations de index.css font bouger quand la case entre dans l'écran.
 * Sans elles le dessin reste juste, simplement immobile.
 */
...
/** Une photo de travers qui se redresse en page droite. */
export function IllustrationCadrage() {
  ...
  return (
    <svg viewBox="0 0 260 120" className={cadre} role="img" aria-label="Photo redressée">
      ...
      <g className="at-fondu">
        <path d="M124 62 h16" stroke={GREEN} strokeWidth="2" strokeLinecap="round" fill="none" />
        <path
          d="M136 57 l6 5 -6 5"
          stroke={GREEN}
          strokeWidth="2"
...
/** Un document dont trois informations ressortent, une à une. */
export function IllustrationLecture() {
  ...
  return (
    <svg viewBox="0 0 260 120" className={cadre} role="img" aria-label="Informations reconnues">
      ...
      {champs.map(({ y, t }, i) => (
        <g key={t} className={`at-pastille at-d${i + 1}`}>
          <path d={`M144 ${y + 9} H164`} stroke={GREEN} strokeWidth="1.2" strokeDasharray="3 3" />
          <rect x="164" y={y} width="72" height="18" rx="9" fill={GREEN} />
          <text
            x="200"
            y={y + 12.5}
            textAnchor="middle"
...
/** Le compte à rebours des rappels, qui s'égrène. */
export function IllustrationRappels() {
  ...
  return (
    <svg viewBox="0 0 260 120" className={cadre} role="img" aria-label="Rappels avant l’échéance">
      ...
      <path
        className="at-trace"
        style={{ '--longueur': 116 } as CSSProperties}
        d="M34 62 H150"
        stroke={GREEN}
        strokeWidth="3"
        strokeLinecap="round"
      ...
      {jalons.slice(0, 3).map(({ x }, i) => (
        <g key={`c${x}`} className={`at-cloche at-d${i + 2}`} transform={`translate(${x - 6} 30)`}>
          <path
          ...
          />
          <path
            d="M4.4 15.5 a1.8 1.8 0 0 0 3.2 0"
            stroke={GREEN}
            strokeWidth="1.4"
            fill="none"
            strokeLinecap="round"
...
/** Une série de factures dont la dernière décroche. */
export function IllustrationHausse() {
  ...
  return (
    <svg viewBox="0 0 260 120" className={cadre} role="img" aria-label="Hausse détectée">
      ...
      {barres.map(({ x, h, hausse }, i) => (
        <rect
          key={x}
          className={`at-barre at-d${i + 1}`}
          x={x}
          y={96 - h}
          width="26"
          height={h}
